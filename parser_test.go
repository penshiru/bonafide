...
func TestPrepareTags(t *testing.T) {
	tag1 := []model.Tag{
		model.Tag{Name: "Name", Regex: "LEY DE|CÓDIGO"},
		model.Tag{Name: "Number", Regex: "No\\."},
	}
	tag2 := []model.Tag{
		model.Tag{Name: "Aproved", Regex: "Aprobada"},
		model.Tag{Name: "Diary", Regex: "Publicada"},
		model.Tag{Name: "Arto", Regex: "Art\\.\\s\\d+"},
	}
	tag3 := []model.Tag{
		model.Tag{Name: "Diary", Regex: "Publicada"},
	}
	...
	for _, tt := range tests {
		...
		if actual[0].exp.String() != tt.out {
			t.Errorf("Tag(%v): expected %v, actual %s", tt.in, tt.out, actual[0].exp.String())
			break
		}
	}
...
func TestFindBasicData(t *testing.T) {
	...
	for _, tt := range journalTests {
		...
		if law.Journal != tt.out {
			t.Errorf("Name testing %q, expected %q, actual %s", tt.in, tt.out, law.Journal)
		}
	}
