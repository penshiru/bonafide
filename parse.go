...
func ParseConcurrent(uri string) *model.Law {
...
func FindBasicData(law *model.Law, done chan<- struct{}, in <-chan string, wg *sync.WaitGroup) {
	wg.Add(1)

	iTags := model.NewIntroTags()
...
func makeLaw(lines []string, law *model.Law, tag <-chan foundTag,
	index <-chan []int, wg *sync.WaitGroup) *model.Stack {

	mStack := model.NewStack(3) //estimated stack size
	tags := <-index
	...
	for t := range tag {
		switch t.tagname {
		case "Libro":
			mStack.Push(model.Book{Name: lines[t.line], Reviewed: false})
		case "Titulo":
			// fmt.Println("Titulo ", lines[t.line])
			mStack.Push(model.Title{Name: lines[t.line], Reviewed: false})

		case "Capitulo":
			// fmt.Println("Capitulo ", lines[t.line])
			mStack.Push(model.Chapter{Name: lines[t.line], Reviewed: false})
...
func jsonFormat2(stack *model.Stack, mLaw *model.Law) *model.Law {
		...
		case model.Article:
			...
			if hasBook {
				...
				if hasTitle {
					...
					if currentChapter == -1 {
						element := model.Chapter{Name: "No Title"}
						mLaw.Titles[currentTitle].Chapters = mLaw.Titles[currentTitle].AddChapter(element)
						currentChapter++
					}
...
func prepareTags(tags []model.Tag) []preparedTag {
	// t := time.Now()
...
func fillBasicData(tag string, value string, law *model.Law, matches map[int]*regexp.Regexp) {
...
func feedArticle(lines []string, last int, t foundTag, tags []int, r int) model.Article {
	article_txt := []string{}
	mArticle := model.Article{Name: lines[t.line-1], Reviewed: false}
